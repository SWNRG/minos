...
for filename in files :
	...
	# new file with every round statistics
	writeFileRoundPDR = open(folder2write + "/" + "roundPDR-"+writeFileName+".dat", "w+")
	writeFileRoundPDR.write('#time(min)   pdr\n')
	#================= Basic Files for ALL NODES STATS =======================
	...
	mobNodesNumber = 7 #one more than the last mobile number
	...
	with open(readFile, "r") as fp:
		...
		udp_recv_Round=0 # received for every round
	...
	#=================== THIS IS FOR EVERY NODE, FOR EVERY ROUND =============
					#print("R:"+str(r_counter)+", node:"+str(currNode)+", packts sent:"+str(udp_sent))
					#print(r_counter, "sent/received:",udp_sent,udp_recv_Total)
	# =========================================================================
...
# =================== UNIQUE time series for each node =================================
				...
				#print (r_counter)
				# print("nodes:               ", nodes)
				#print("roundPacketsSent:    ", roundPacketsSent)
				#print("roundPacketsReceived:", roundPacketsReceived)
				#print
				#print("nodePacketsSent:     ", nodePacketsSent)
				#print("nodePacketsReceived: ", nodePacketsReceived)
				#print
				curStatsSent=[nodePacketsSent[i] - roundPacketsSent[i] for i in xrange(min(len(roundPacketsSent), len(nodePacketsSent)))]
				curStatsRecv=[nodePacketsReceived[i] - roundPacketsReceived[i] for i in xrange(min(len(roundPacketsReceived), len(nodePacketsReceived)))]
				curStatsSent=sum(curStatsSent)
				curStatsRecv=sum(curStatsRecv)
				#print ("curStatsSent: ",curStatsSent)
				#print ("curStatsRecv: ",curStatsRecv)

				if(curStatsSent==0):
					curStatsSent = 1.0
				everyRoundPDR = truncate(100*(curStatsRecv / float(curStatsSent)) ,2)

				line2write = str(r_counter)+" "+ str( everyRoundPDR)
				#print line2write
				#print("======= Writing to writeFileRoundPDR =============")
				writeFileRoundPDR.write(line2write+"\n")

				# assign the previous values to subtract in the next round
				roundPacketsSent = list( nodePacketsSent) #copy
				roundPacketsReceived = list(nodePacketsReceived) #copy MAMATAS!!!!
