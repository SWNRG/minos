...
# ========= EXTRA FILE(S) FOR MOBILE NODES STATS ONLY ====================
	writeMobFileName = "mobs_only_"+file2Scan
	...
	# =============New Entry: For EACH NODE, write the sent packets========================
					...
					if currNode>1:
						if (currNode not in nodes): # or currNode==1:
							#print("New node:" + str(currNode))
							#print("old nodes:",nodes)
							nodes.append(currNode)  # expand the array with another new node
							#print("new nodes:",nodes)
							nodePacketsSent.append(1)  # expand the array
							nodePacketsReceived.append(0)
						else:
							index2 =nodes.index(currNode)
							#print("node exists:" + str(currNode))
							#nodePacketsSent.append(0)#the other array is already expanded above
							# increase the 2nd array by one
							nodePacketsSent[index2] = nodePacketsSent[index2] + 1
							#print
							#print("nodes: ", nodes)
...
#************************* NEXT ROUND ******************************
		...
		print("Writing to mobONLYFile:"+str(writeMobFileName))
