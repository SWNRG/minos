...
extern uint8_t node_color; //in rpl.h

extern uint32_t aggregator; // in rpl.h
...
/*********** Sending back the received message *********************/
  ...
  printf("DATA: In p msg '%s' recvd, sent  BACK to ",data);
  uip_debug_ipaddr_print(sender_addr);
  printf("\n");
  int tt = 
  		simple_udp_sendto(&unicast_connection, data, strlen(data) + 1, 			sender_addr);
	printf("Return from sender: '%d'\n",tt);
/********************************************************************/
}
/*---------------------------------------------------------------------------*/
