...
#ifndef PERIOD
//PERIOD ALTERED by EXTERNAL PROGRAM
//PERIOD ALTERED by EXTERNAL PROGRAM
//PERIOD ALTERED by EXTERNAL PROGRAM
//PERIOD ALTERED by EXTERNAL PROGRAM
#define PERIOD 50
...
PROCESS_THREAD(print_metrics_process, ev, data){
  ...
  // BECAREFUL: In dynamic mode you can decide when and where to change it
  ...
  PRINTF("Printing Client Metrics...\n");
	...
	// mode =2 means LEAF NODE i.e., node does not accept forwarding packets
	rpl_set_mode(2);
  ...
  while(1) {
	...
	//Mobile nodes are not suppose to change Imin. BUT YOU CAN TEST IT


	// Dynamic mode . Changing Imin when counter ==30
	/*
    if(counter == 30){
       printf("R:%d, Changing Imin from 12 to 8\n",counter);
       d->instance->dio_intmin = 8;

			//this is Tryfon's NEW VARIABLE, NOT Idouble!!
		  //d->instance->dio_intcurrent = 8;
		  
		  //George testing set the Idouble to an absolute minimum
