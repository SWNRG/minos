...
PROCESS_THREAD(print_metrics_process, ev, data){
  ...
  while(1) {
   ...
   printf("R:%d, SINK, DAG-VERSION:%d\n",counter, d->version); 
   printf("R:%d, SINK, DAG-DIO_COUNTER:%d\n",counter, d->instance->dio_counter); 
   printf("R:%d, SINK, DAG-Imin:%d\n",counter, d->instance->dio_intmin); 
   printf("R:%d, SINK, DAG-IDOUBLINGS:%d\n",counter, d->instance->dio_intdoubl); 
   printf("R:%d, SINK, DAG-IminCURRENT:%d\n",counter, d->instance->dio_intcurrent); 
   printf("R:%d, SINK, DAG-DefaultLIFETIME:%d\n",counter, d->instance->default_lifetime); 
   printf("R:%d, SINK, DAG-LIFETIME:%d\n",counter, d->instance->lifetime_unit); 
