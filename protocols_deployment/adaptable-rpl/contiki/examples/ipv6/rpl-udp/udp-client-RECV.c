...
set_global_address(void)
{
  ...
  uip_ip6addr(&server_ipaddr, UIP_DS6_DEFAULT_PREFIX, 0, 0, 0, 0, 0x00ff, 0xfe00, 4);
  
  //uip_ip6addr(&ipaddr, UIP_DS6_DEFAULT_PREFIX, 0, 0, 0, 0, 0x00ff, 0xfe00, 4);
...
*/
  ...
  while(1) {
    PROCESS_YIELD();
    if(ev == tcpip_event) {
      printf("Server-node: incoming msg...");
      tcpip_handler();
    }
